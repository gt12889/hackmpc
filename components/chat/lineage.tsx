  ...
  return (
    <li className="relative pl-5">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <code className="rounded bg-background/60 px-1 py-0.5 text-[10px] text-muted-foreground">{t.name}</code>
        {frame.map((f, i) => (
        ...
        {filterSets.map((set, si) => {
          ...
          return (
            <div key={si} className="flex flex-wrap items-center gap-1.5">
              {set.label && (
                <span className="text-[10px] font-medium text-muted-foreground/80">{set.label}:</span>
              )}
              {tokens.map((tok, i) => (
      ...
      {t.ok && t.sample && t.sample.length > 0 && (
        <table className="mt-1.5 w-full max-w-md border-separate border-spacing-0 text-[10px]">
          <tbody>
            {t.sample.map((row, i) => {
...
export function Lineage({ tools }: { tools: ToolCallTrace[] }) {
  ...
  return (
    <div className="w-full rounded-lg border border-border/70 bg-card/50">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-1.5 px-3 py-2 text-left text-[11px] font-medium text-muted-foreground transition-colors hover:text-foreground"
      >
      ...
      {open && (
        <ol className="space-y-3 border-t border-border/60 px-3 py-3">
          ...
          <li className="pl-5 text-[10px] italic text-muted-foreground/70">
