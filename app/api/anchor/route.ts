...
const TYPES: RecordType[] = ["report", "request", "alert", "vendor"];
...
export async function GET(req: NextRequest) {
  ...
  if (recordType || recordId) {
    if (!isType(recordType) || !recordId) {
      return NextResponse.json({ error: "recordType (report|request|alert|vendor) and recordId required" }, { status: 400 });
    }
